...
`npm run deploy:prod` fails if the production checkout is not clean, then pulls the latest code, restarts itself when the checkout changed, installs dependencies, builds the app, stops the API service, applies pending checked-in Prisma migrations, backfills missed closed-day scores, publishes the frontend files, refreshes the nginx static cache policy, installs and starts worker timers, starts the API service, and checks the health endpoint.
...
Production deploy installs and enables these worker timers. The backup timer is still managed separately because it requires `/etc/life-os/backup.env`.

```bash
sudo systemctl enable --now life-os-postgres-backup.timer
```

Manual score-history backfill, if needed outside deploy:
