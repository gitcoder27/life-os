...
type NotificationGenerationTargetUser = {
  id: string;
...
export async function generateRuleNotifications(
  ...
  for (const user of users) {
    const result = await generateRuleNotificationsForTargetUser(prisma, user, now);
...
export async function generateRuleNotificationsForUser(
  prisma: PrismaClient,
  userId: string,
  now: Date,
): Promise<NotificationGenerationResult> {
  ...
  return generateRuleNotificationsForTargetUser(prisma, { id: userId, preferences }, now);
}

async function generateRuleNotificationsForTargetUser(
  prisma: PrismaClient,
  user: NotificationGenerationTargetUser,
  now: Date,
): Promise<NotificationGenerationResult> {
